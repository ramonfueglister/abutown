...
describe('pak128 catalog', () => {
  ...
  const yellowCrossingRoadPath = '/simutrans-assets/pak128/infrastructure/roads/yellow_crossing_hk_road_090.png';
  const yellowCrossingRoadProvenance = {
    sourcePath: 'infrastructure/roads/yellow_crossing_hk_road_090.png',
    datPath: 'infrastructure/roads/yellow_crossing_hk_road_090.dat',
  };
  const yellowSourceDir = join(root, 'public', 'simutrans-assets', 'pak128', 'infrastructure', 'roads', 'yellow-crossing-hk', 'source');
  const yellowRuntimePng = join(root, 'public', 'simutrans-assets', 'pak128', 'infrastructure', 'roads', 'yellow_crossing_hk_road_090.png');
  const yellowRuntimeDat = join(root, 'public', 'simutrans-assets', 'pak128', 'infrastructure', 'roads', 'yellow_crossing_hk_road_090.dat');
  ...
  it('has generated Yellow Crossing runtime and source provenance', () => {
    ...
    expect(dat).not.toContain('infrastructure/roads/road_090.png');
    ...
    expect(manifest.noFallbackSource).toContain('road_090.png');
  });
});
