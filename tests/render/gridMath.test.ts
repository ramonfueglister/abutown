...
describe('gridMath', () => {
  ...
  it('projects a signed forward offset along a screen vector', () => {
    expect(screenForwardOffset({ x: 0, y: 0 }, { x: 3, y: 4 }, 10)).toEqual({ x: 6, y: 8 });
    expect(screenForwardOffset({ x: 2, y: 2 }, { x: 2, y: 2 }, 10)).toEqual({ x: 0, y: 0 });
    expect(screenForwardOffset({ x: 0, y: 0 }, { x: 0, y: 1 }, -5)).toEqual({ x: -0, y: -5 });
  });
});
