import { describe, expect, it } from 'vitest';
import {
  carRenderStyle,
  carVisualWorldPoint,
  pedestrianRenderStyle,
} from '../../src/render/entityRenderStyle';

describe('entityRenderStyle', () => {
  it('computes car selection ring, angle, and capsule size from projected points', () => {
    expect(carRenderStyle({ x: 0, y: 0 }, { x: 10, y: 0 }, 0.5)).toEqual({
      angle: 0,
      selection: { x: 28, y: 20 },
      capsule: { length: 32, width: 12.8 },
    });
  });

  it('clamps car metrics when the camera is zoomed far out', () => {
    expect(carRenderStyle({ x: 0, y: 0 }, { x: 0, y: 10 }, 0.2)).toEqual({
      angle: Math.PI / 2,
      selection: { x: 36, y: 28 },
      capsule: { length: 44, width: 19 },
    });
  });

  it('keeps car visual points lane-separated and id-spread along the movement vector', () => {
    expect(carVisualWorldPoint({
      id: 'vehicle:1',
      path: [{ x: 0, y: 0 }, { x: 1, y: 0 }],
      offset: 0,
      speed: 0,
      direction: 'e',
      sprite: { sheet: 'minimal-cars.0', role: 'vehicle.0' },
    }, 0.5, { width: 10, height: 10 })).toEqual({
      x: 21.8,
      y: 18.6,
    });
  });

  it('does not add a default visual lane offset for backend pedestrians', () => {
    expect(pedestrianRenderStyle({ x: 0, y: 0 }, { x: 10, y: 0 }, 0.5, 0)).toEqual({
      lane: { x: 0, y: 0 },
      selectedRadius: 9,
      radius: 3.2,
    });
  });

  it('can still apply an explicit pedestrian lane offset', () => {
    expect(pedestrianRenderStyle({ x: 0, y: 0 }, { x: 10, y: 0 }, 0.5, 2)).toEqual({
      lane: { x: 0, y: 4 },
      selectedRadius: 9,
      radius: 3.2,
    });
  });

  it('returns zero pedestrian lane for stationary agents', () => {
    expect(pedestrianRenderStyle({ x: 4, y: 4 }, { x: 4, y: 4 }, 0.5, 3).lane).toEqual({ x: 0, y: 0 });
  });
});
