...
test('renders the city with a bounded fixed-map camera', async ({ page }) => {
  ...
  expect(state.city.vehicleDiagnostics.pathTilesOffRoad).toBe(0);
  expect(state.city.vehicleDiagnostics.pathTilesOnRails).toBe(0);
  expect(state.city.vehicleDiagnostics.teleportingVehiclePaths).toBe(0);
  expect(state.city.vehicleDiagnostics.illegalVehicleUTurnPaths).toBe(0);
