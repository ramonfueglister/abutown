// Look-loop capture harness: spawn vite dev, open /look.html headless,
// wait for first rendered frame, screenshot to artifacts/look/<name>.png.
// Usage: node scripts/capture-look.mjs [name]
...
const PORT = 5175;
const NAME = process.argv[2] ?? 'morning';
const PRESET = process.argv[3] ?? 'morning';
const CAM = process.argv[4] ?? 'default';
const OUT = `artifacts/look/${NAME}.png`;
...
try {
  if (!(await waitForPort(30000))) {
    ...
    mkdirSync('artifacts/look', { recursive: true });
    ...
    await page.goto(`http://${HOST}:${PORT}/look.html?preset=${PRESET}&cam=${CAM}`, { waitUntil: 'load', timeout: 20000 });
    try {
      ...
      await page.screenshot({ path: `artifacts/look/${NAME}-broken.png` }).catch(() => {});
    }
    if (errors.length) {
