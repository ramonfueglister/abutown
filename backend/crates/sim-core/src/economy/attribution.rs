...
pub fn run_citizen_attribution_system(world: &mut World) {
    ...
    // diagnostic — only fires when `Tick` is present (a minimal test world without it
    // simply emits no heartbeat, rather than a misleading "tick 0" line or a panic).
    if let Some(tick) = world
