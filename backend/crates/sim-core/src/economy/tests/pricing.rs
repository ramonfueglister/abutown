...
fn sustained_shortage_raises_price_monotonically_and_bounded_over_intervals() {
    use crate::economy::{EconomicActorId, GOOD_TOOLS, MarketGoodKey, MarketGoods, MarketId, DemandPools, SupplyPools};
    let m = MarketId(1);
    let cfg = EconomyConfig::default();
    let mut d = DemandPools(std::collections::BTreeMap::from([(EconomicActorId(1), demand_pool(1, m, 2_000))]));
    let mut s = SupplyPools::default();
    let mut g = MarketGoods::default();
    ...
    assert!(*prices.last().unwrap() <= 2_000 + 2_000 * 9 / 100, "rise is speed-limited: {prices:?}");
