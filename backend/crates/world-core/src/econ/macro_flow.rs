...
use crate::econ::orders::{drain_residual_ask, drain_residual_bid};
use crate::econ::pools::affordable_qty;
use crate::econ::producers::{InputPools, ProducerPolicies, participation_bound, scaled_batch_qty};
use crate::econ::{
...
pub fn run_macro_flow_at_tick(
    ...
    config: &EconomyConfig,
    current_tick: u64,
    shipments: &mut crate::econ::FlowShipments,
    next_shipment_id: &mut crate::econ::NextShipmentId,
    realized: &mut crate::econ::RealizedFlows,
    orders: &mut crate::econ::OrderBook,
    next_order_id: &mut crate::econ::NextOrderId,
    receipts: &mut BTreeMap<(EconomicActorId, MarketId), Money>,
    outlays: &mut BTreeMap<(EconomicActorId, MarketId), Money>,
) -> Result<(), EconomyError> {
    realized.0.clear();
        ...
        match settle_flow_with_receipts(
            ...
            Ok(event) => {
                ...
                if eflow.src != eflow.dst {
                    ...
                    let travel_ticks =
                        crate::econ::flow_shipments::shipment_travel_ticks(eflow.dist, config);
                    shipments.0.insert(
                        id,
                        crate::econ::FlowShipment {
                            id,
                            from_market: eflow.src,
                            to_market: eflow.dst,
                            good: eflow.good,
                            qty: crate::econ::Quantity(eflow.q),
                ...
                if eflow.q > 0 {
                    realized.0.push(crate::econ::RealizedFlow {
                        src: eflow.src,
                        dst: eflow.dst,
                        good: eflow.good,
                        qty: eflow.q,
