...
**Intra-Set-Ordering erzwungen (Fix D2):** `run_distribute_profit_system` läuft im PayWages-Set mit **expliziter `.after(run_pay_wages_system)`-Kante** (in einem SEPARATEN `add_systems`-Aufruf registriert — die Bevy-0.18-Gotcha: eine Intra-Tupel-`.after` plus Tupel-`.before` erzwingt die Reihenfolge NICHT zuverlässig), der Rebate `.after` beidem. Die deterministische Reihenfolge der drei Schreiber von `DemandPools.income_last_tick`/`AccountBook` ist durch diese expliziten Kanten + die Set-Chain garantiert und durch einen **Recorder-Ordering-Test** verifiziert (der die tatsächlich gelaufene Reihenfolge gegen `wage → profit → rebate` prüft). Ein whole-schedule `ambiguity_detection: LogLevel::Error` ist KEIN Commit-Gate (er würde auf jede vorbestehende Ambiguität im CorePlugin+MobilityPlugin+EconomyPlugin-Schedule paniken — eine unbekannte globale Eigenschaft); ein optionaler, NUR auf die drei Schreiber gescopter Ambiguity-Probe ist informational. Reihenfolge: `wage-reset → wage-credit → profit-credit → rebate-credit`.
...
`goods.rs` (GOOD_RAW), `production.rs` (EXTRACTOR/RawDeposit/RawDeposits/run_regen_at_tick), `wages.rs` (dividend_share_bps-Validierung), `transport.rs` (run_transport_rebate_at_tick), `ledger.rs` (Regenerated/ProfitDistributed/TransportRebate + event_type-Arme), `systems.rs` (Regenerate + TransportRebate Sets, Profit-in-PayWages mit .after in separatem add_systems-Aufruf + Recorder-Ordering-Test), `seed.rs` (EXTRACTOR-Seed), `persist.rs` (raw_deposits), `mod.rs` (RawDeposits registrieren), `tests/{conservation,production,persist}.rs`.
