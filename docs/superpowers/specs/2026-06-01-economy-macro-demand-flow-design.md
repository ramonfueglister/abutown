...
Settle each accepted flow against the clones; if a single edge errors (overflow on a pathological qty/distance), **skip that edge, emit `EconomyEvent::MarketClearFailed { market, good, reason }`, and continue** — one poisoning market must not freeze the whole dormant economy. The system wrapper **must stop swallowing the `Result`** (today `let _ = run_warm_market_flow_at_tick(...)`): on `Err` it emits an audit event. The atomic boundary is preserved per-edge.
...
4. **`prices_converge_to_within_transport_cost`** — A cheap, B dear, persistent supply@A/demand@B. Loop N intervals; assert on **`last_settlement_price`** that the gap is monotone non-increasing and converges to `≤ unit_transport_cost + Money(1)`.
...
10. **`poisoning_market_does_not_abort_others`** — two pairs, one with an overflow input; assert the healthy pair still flows + conserves, and `MarketClearFailed` is emitted for the bad one.
...
**Edge cases (each its own `#[test]`):** `no_demand_no_flow`/`no_supply_no_flow`; `single_market_no_partner`; `zero_distance_markets` (transport 0 → full equalization, conserves); `overflow_returns_error_not_panic` (errored edge leaves books unchanged); `tiny_qty_floors_to_zero`; `zero_price_band_market_skipped` (the `p_m.0 <= 0` guard).
