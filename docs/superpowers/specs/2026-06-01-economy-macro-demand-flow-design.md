...
Settle each accepted flow against the clones; if a single edge errors **at settlement time** — e.g. a deficit market imported from two sources in one interval and the second edge's `lock_cash` exceeds the buyer's bucket-time affordability (budgets are computed once against the live books per STEP A — an acceptable LOD under-approximation), or any checked op underflows — **skip that edge, emit `EconomyEvent::MarketClearFailed { market, good, reason }`, and continue**; one poisoning edge must not freeze the whole dormant economy. (Gate-time arithmetic faults are already pruned in STEP D, so this path handles **settlement** faults only.) The system wrapper **must stop swallowing the `Result`** (today `let _ = run_warm_market_flow_at_tick(...)`): on `Err` it emits an audit event. The atomic boundary is preserved per-edge — a faulted edge contributes nothing, healthy edges commit.
...
10. **`poisoning_market_does_not_abort_others`** — construct a **settle-time** fault (e.g. one deficit buyer targeted by two surplus sources in one interval, so the second edge's `lock_cash` exceeds its bucket-time affordability) alongside a healthy pair. Assert the healthy pair still flows + conserves, the faulted edge moves nothing (books byte-identical for it), and `MarketClearFailed` is emitted for it.
