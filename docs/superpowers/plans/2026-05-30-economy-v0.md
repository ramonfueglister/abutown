...
4. **Wiring verification** notes (Task 8/9): confirm the `tick_increment_system`
   path + that exclusive-system/`SystemSet`/cross-plugin `.before(..)` composes,
   and that the constructor used by `runtime_installs_economy_plugin` actually
   installs `EconomyPlugin`.
...
use crate::economy::{
    ...
    MarketGoods, OrderBook, SupplyPools, TradeLedger,
};
use crate::mobility::resources::Tick;
...
pub fn expire_orders_system(world: &mut World) {
    let tick = world.resource::<Tick>().0;
    world.resource_scope(|world, mut accounts: Mut<AccountBook>| {
        world.resource_scope(|world, mut inventory: Mut<InventoryBook>| {
            world.resource_scope(|world, mut orders: Mut<OrderBook>| {
                world.resource_scope(|world, mut ledger: Mut<TradeLedger>| {
                    world.resource_scope(|_, mut dirty: Mut<DirtyMarketGoods>| {
                        let _ = expire_orders_at_tick(&mut accounts, &mut inventory, &mut orders, &mut ledger, &mut dirty, tick);
                    });
                });
            });
        });
...
pub fn generate_pool_orders_system(world: &mut World) {
    let tick = world.resource::<Tick>().0;
    let ttl = world.resource::<EconomyConfig>().default_order_ttl_ticks;
    world.resource_scope(|world, mut accounts: Mut<AccountBook>| {
        world.resource_scope(|world, mut inventory: Mut<InventoryBook>| {
            world.resource_scope(|world, mut orders: Mut<OrderBook>| {
                world.resource_scope(|world, mut ledger: Mut<TradeLedger>| {
                    world.resource_scope(|world, mut dirty: Mut<DirtyMarketGoods>| {
                        world.resource_scope(|world, mut next: Mut<crate::economy::NextOrderId>| {
                            world.resource_scope(|world, mut demand: Mut<DemandPools>| {
                                world.resource_scope(|_, mut supply: Mut<SupplyPools>| {
...
pub fn clear_dirty_markets_system(world: &mut World) {
    let tick = world.resource::<Tick>().0;
    let keys: Vec<_> = world.resource::<DirtyMarketGoods>().0.iter().copied().collect();
    world.resource_mut::<DirtyMarketGoods>().0.clear();
    for key in keys {
        world.resource_scope(|world, mut accounts: Mut<AccountBook>| {
            world.resource_scope(|world, mut inventory: Mut<InventoryBook>| {
                world.resource_scope(|world, mut orders: Mut<OrderBook>| {
                    world.resource_scope(|world, mut ledger: Mut<TradeLedger>| {
                        world.resource_scope(|_, mut goods: Mut<MarketGoods>| {
                            if let Err(reason) = clear_market_good(&mut accounts, &mut inventory, &mut orders, &mut ledger, &mut goods, key, tick) {
...
pub fn update_market_telemetry_system(world: &mut World) {
...
> **Verify against real code (do before relying on the ordering):**
> - Confirm the tick-increment system path. In this repo it lives in
>   `mobility/systems/bookkeeping.rs`; check it is reachable as
>   `crate::mobility::systems::tick_increment_system` (re-exported) — if not, use
>   the actual public path. If the economy systems do not strictly need to run
>   before the increment (they only read `Tick`), the `.before(..)` can be dropped;
>   the `economy_clears_a_trade_end_to_end` test still pins behaviour either way.
> - These four systems are **exclusive** (`fn(&mut World)`). Confirm exclusive
>   systems + `EconomySet` `.chain()` + a cross-plugin `.before(<parallel system>)`
>   compose in the existing `Schedule` (the `economy_plugin_installs_*` and
>   end-to-end tests will catch a mis-composition at `schedule.run`).
